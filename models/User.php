...
class User {
    ...
    public function generarNumeroRegistro() {
        ...
        for ($i = 0; $i < $maxIntentos; $i++) {
            // Generar número aleatorio de 6 dígitos (000001 a 999999)
        ...
        // Si después de 100 intentos no encuentra uno único, usar timestamp
        return 'REG-' . date('YmdHis') . rand(10, 99);
    }
    ...
    // REGISTRO COMPLETO CON NÚMERO DE REGISTRO
    public function registerComplete($data, $huella = null) {
        $numeroRegistro = $this->generarNumeroRegistro();
        
        $sql = "INSERT INTO usuarios (numero_registro, nombres, apellidos, carnet, fecha_nacimiento, direccion, telefono, email, password, huella_digital, rol, habilitado_voto, ya_voto, activo) 
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'usuario', ?, 0, 1)";
    ...
    // Obtener usuario por número de registro
