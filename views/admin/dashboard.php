...
<head>
    ...
    <style>
        * { margin:0; padding:0; box-sizing:border-box; }
        ...
        .card-box h3 { color:#fff; font-weight:800; font-size:16px; margin-bottom:6px; display:flex; align-items:center; gap:9px; }
        .card-box h3 i { color:#FF6B00; }
        .estado-badge { display:inline-flex; align-items:center; gap:8px; padding:6px 14px; border-radius:20px; font-size:12px; font-weight:700; margin-bottom:16px; }
        .estado-activo { background:rgba(39,174,96,0.15); color:#5cdb95; border:1px solid rgba(39,174,96,0.3); }
        .estado-inactivo { background:rgba(231,76,60,0.15); color:#ff6b6b; border:1px solid rgba(231,76,60,0.3); }
...
<body>
    ...
    <div class="main-content">
        ...
        <!-- Control de Votación -->
        <div class="card-box">
            <h3><i class="fas fa-toggle-on"></i> Control de Votación</h3>
            ...
            <p style="color:rgba(255,255,255,0.4);font-size:13px;margin-bottom:20px;">
                Ciudadanos habilitados: <strong style="color:#FF8C38;"><?= $habilitados ?> de <?= $totalUsuarios ?></strong>
            </p>

            <form method="POST" style="display:flex;gap:14px;flex-wrap:wrap;">
                <button type="submit" name="accion_votacion" value="habilitar"
                    style="background:#27AE60;color:#fff;border:none;padding:12px 24px;border-radius:10px;font-size:14px;font-weight:800;cursor:pointer;display:inline-flex;align-items:center;gap:8px;transition:.2s;"
                    onmouseover="this.style.background='#2ecc71'" onmouseout="this.style.background='#27AE60'"
                    onclick="return confirm('¿Habilitar votación y registro para TODOS?')">
                    <i class="fas fa-unlock"></i> Habilitar Votación para Todos
                </button>
                <button type="submit" name="accion_votacion" value="deshabilitar"
                    style="background:rgba(231,76,60,0.15);color:#ff6b6b;border:1px solid rgba(231,76,60,0.3);padding:12px 24px;border-radius:10px;font-size:14px;font-weight:800;cursor:pointer;display:inline-flex;align-items:center;gap:8px;transition:.2s;"
                    onmouseover="this.style.background='rgba(231,76,60,0.3)'" onmouseout="this.style.background='rgba(231,76,60,0.15)'"
                    onclick="return confirm('¿Deshabilitar votación y registro para TODOS?')">
        ...
        <div class="card-box">
            <h3><i class="fas fa-clock"></i> Configuración de Horario de Votación</h3>
            <form method="POST" style="display:grid;grid-template-columns:repeat(auto-fit,minmax(180px,1fr));gap:16px;align-items:end;">
                <div>
                    ...
                    <input type="date" name="fecha_votacion" value="<?= htmlspecialchars($fechaVotacion) ?>"
                        min="2026-01-01" max="2030-12-31" onkeydown="return false"
